# This code is inspired from https://github.com/chakki-works/seqeval

from typing import Optional, List, Type, Set, Tuple
...
class Entity:
    ...
    def __repr__(self):
        return f'({self.sent_id}, {self.start}, {self.end}, {self.tag})'
...
class Token:
    ...
    def check_patterns(self, prev: "Token", patterns: Set[Tuple[Prefix, Prefix,
                                                                Tag]]) -> bool:
        """Check whether the prefix patterns are matched."""
        for prev_prefix, current_prefix, tag_cond in patterns:
            if prev.prefix in prev_prefix and self.prefix in current_prefix and self.check_tag(
                    prev, tag_cond):
                return True
        return False
...
class IOE1(Token):
    # Todo: IOE1 hasn't yet been able to handle some cases. See unit testing. (https://github.com/chakki-works/seqeval)
    allowed_prefix = Prefix.I | Prefix.O | Prefix.E
    start_patterns = {(Prefix.O, Prefix.I, Tag.ANY),
                      (Prefix.I, Prefix.I, Tag.DIFF),
                      (Prefix.E, Prefix.I, Tag.ANY),
                      (Prefix.E, Prefix.E, Tag.SAME)}
    inside_patterns = {(Prefix.I, Prefix.I, Tag.SAME),
...
class IOB2(Token):
    allowed_prefix = Prefix.I | Prefix.O | Prefix.B
    start_patterns = {(Prefix.ANY, Prefix.B, Tag.ANY)}
    inside_patterns = {(Prefix.B, Prefix.I, Tag.SAME),
                       (Prefix.I, Prefix.I, Tag.SAME)}
    end_patterns = {(Prefix.I, Prefix.O, Tag.ANY),
                    (Prefix.I, Prefix.I, Tag.DIFF),
                    (Prefix.I, Prefix.B, Tag.ANY),
                    (Prefix.B, Prefix.O, Tag.ANY),
                    (Prefix.B, Prefix.I, Tag.DIFF),
...
class IOE2(Token):
    ...
    start_patterns = {(Prefix.O, Prefix.I, Tag.ANY),
                      ...
                      (Prefix.I, Prefix.I, Tag.DIFF),
                      (Prefix.I, Prefix.E, Tag.DIFF)}
    inside_patterns = {(Prefix.I, Prefix.E, Tag.SAME),
                       (Prefix.I, Prefix.I, Tag.SAME)}
    end_patterns = {(Prefix.E, Prefix.ANY, Tag.ANY)}
...
class IOBES(Token):
    ...
    inside_patterns = {(Prefix.B, Prefix.I, Tag.SAME),
                       ...
                       (Prefix.I, Prefix.I, Tag.SAME),
                       (Prefix.I, Prefix.E, Tag.SAME)}
    end_patterns = {(Prefix.S, Prefix.ANY, Tag.ANY),
...
class BILOU(Token):
    ...
    end_patterns = {(Prefix.U, Prefix.ANY, Tag.ANY),
...
class Tokens:
    ...
    def __init__(self,
        ...
        self.tokens = [
            scheme(token, suffix=suffix, delimiter=delimiter)
            for token in tokens
        ]
    ...
    def entities(self) -> List[Entity]:
        ...
        while i < len(self.extended_tokens):
            ...
            if token.is_start(prev):
                ...
                if self._is_end(end):
                    entity = Entity(sent_id=self.sent_id,
                                    start=i,
                                    end=end,
                                    tag=token.tag)
    ...
    def _forward(self, start: int, prev: Token) -> int:
    ...
    def __init__(self,
        ...
        self.entities = [
            Tokens(seq,
                   ...
                   suffix=suffix,
                   delimiter=delimiter,
                   sent_id=sent_id).entities
            for sent_id, seq in enumerate(sequences)
        ]

    def filter(self, tag_name: str) -> Set[Entity]:
        entities = {
            entity
            for entity in chain(*self.entities) if entity.tag == tag_name
        }
        return entities

    @property
    def unique_tags(self) -> Set[str]:
        tags = {entity.tag for entity in chain(*self.entities)}
        return tags


def auto_detect(sequences: List[List[str]],
                suffix: bool = False,
                delimiter: str = '-'):
    ...
    allowed_iob2_prefixes = [{Prefix.I, Prefix.O, Prefix.B},
                             {Prefix.I, Prefix.B}, {Prefix.B, Prefix.O},
                             {Prefix.B}]
    allowed_ioe2_prefixes = [{Prefix.I, Prefix.O, Prefix.E},
                             {Prefix.I, Prefix.E}, {Prefix.E, Prefix.O},
                             {Prefix.E}]
...
def get_entities(seq: List[List[str]],
...
def classification_report(
    ...
    Returns:
        target_names: List of all tags found in predictions and targets
        pred_sum: List of predicted positives per tag.
        tp_sum: List of true positives per tag.
        true_sum: List of actual positives per tag.
    """
    ...
    if mode == 'strict':
        ...
        target_names = sorted(entities_true.unique_tags
                              | entities_pred.unique_tags)
    ...
    for type_name in target_names:
        ...
        tp_sum.append(len(entities_true_type & entities_pred_type))
        pred_sum.append(len(entities_pred_type))
        true_sum.append(len(entities_true_type))
